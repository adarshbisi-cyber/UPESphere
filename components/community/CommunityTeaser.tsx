...
const TEASERS = ['Study squads', 'Peer Q&A', 'Campus chatter', 'Notes worth stealing']
  ...
  return (
    <section className="relative min-h-[calc(100svh-4rem)] flex items-center justify-center px-4 sm:px-6 lg:px-8 pt-28 pb-28 overflow-hidden">
      ...
      <div className="pointer-events-none absolute inset-0 -z-10">
        <div className="absolute top-1/4 right-1/3 w-[520px] h-[520px] rounded-full bg-violet-600/15 blur-[130px]" />
        <div className="absolute bottom-1/4 left-1/4 w-[420px] h-[420px] rounded-full bg-indigo-600/12 blur-[130px]" />
      </div>
      ...
      <motion.div variants={stagger} initial="hidden" animate="show" className="max-w-2xl mx-auto text-center">
        ...
        <motion.div variants={item} className="relative w-24 h-24 mx-auto mb-8">
          ...
          <motion.div
            ...
            className="relative w-24 h-24 rounded-3xl bg-gradient-to-br from-violet-500 to-indigo-600 flex items-center justify-center shadow-xl shadow-violet-500/30"
          >
        ...
        {/* status pill */}
        <motion.div variants={item} className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-violet-500/10 border border-violet-500/20 text-violet-300 text-sm font-medium mb-6">
        ...
        <motion.h1 variants={item} className="text-4xl sm:text-6xl font-bold font-display tracking-tight leading-[1.05]">
          Your people are{' '}
          <span className="bg-gradient-to-r from-violet-400 via-indigo-400 to-cyan-400 bg-clip-text text-transparent">gathering</span>.
        </motion.h1>

        <motion.p variants={item} className="text-lg text-muted-foreground mt-5 max-w-xl mx-auto leading-relaxed">
          Study squads, peer Q&amp;A, campus chatter, and notes worth stealing — one place to find your people and
          figure it out together. It&rsquo;s not live yet, but the crew&rsquo;s assembling.
        </motion.p>
        ...
        <motion.div variants={item} className="flex flex-wrap items-center justify-center gap-2.5 mt-10">
          {TEASERS.map(t => (
            <span key={t} className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full text-xs text-foreground/80 border border-white/10 bg-white/[0.03]">
              <Sparkles className="w-3 h-3 text-violet-400" /> {t}
            </span>
          ))}
        </motion.div>
